...
export class BatchDeleteHandler {
  ...
  async handleMessage(body: BatchDeleteMessage): Promise<boolean> {
    try {
      ...
      await new SavedItemDataService({
        dbClient,
        userId: body.userId.toString(),
        apiId: 'backend',
      }).batchDeleteSavedItems(body.itemIds, traceId);
    } catch (error) {
      const message = 'Error occurred during batch delete query';
  ...
  async pollQueue() {
    const params = {
      ...
      MaxNumberOfMessages: config.aws.sqs.listDeleteQueue.maxMessages,
      MessageAttributeNames: ['All'],
      QueueUrl: config.aws.sqs.listDeleteQueue.url,
      VisibilityTimeout: config.aws.sqs.listDeleteQueue.visibilityTimeout,
      WaitTimeSeconds: config.aws.sqs.listDeleteQueue.waitTimeSeconds,
    };
    ...
    if (body != null) {
      ...
      await this.scheduleNextPoll(
        config.aws.sqs.listDeleteQueue.afterMessagePollIntervalSeconds * 1000
      );
    } else {
      // If no messages were found, schedule another poll after a short time
      await this.scheduleNextPoll(
        config.aws.sqs.listDeleteQueue.defaultPollIntervalSeconds * 1000
      );
    }
  }
}
